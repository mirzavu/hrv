    ...
    <div className="w-full max-w-7xl mx-auto font-sans">
      ...
      <div className="bg-white p-3 sm:p-4 md:p-6 rounded-2xl shadow-lg">
        ...
        <div className="flex flex-col xl:flex-row gap-4 xl:gap-8">
          ...
          {/* Sessions View */}
          <div className="w-full xl:w-80 xl:border-l xl:pl-8">
              ...
              {selectedDay && byDate[selectedDay] && byDate[selectedDay].length > 0 ? (
                byDate[selectedDay]
                  ...
                  .map(s => {
                    ...
                    return (
                      <div className="p-3 sm:p-4 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 transition-colors" key={s.id}>
                        <div className="flex justify-between items-center">
                          <p className="font-semibold text-gray-800 text-sm sm:text-base">{s.time}</p>
                          <p className="text-xs sm:text-sm font-medium text-gray-900">{s.durationMin} <span className="text-xs text-gray-500">min</span></p>
                        </div>
                        
                        <div className="flex justify-between items-center">
                          <div className="flex items-center gap-2">
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${hrvStatus.bgColor} ${hrvStatus.color}`}>
                              {hrvStatus.status}
                            </span>
                            <span className="text-xs text-gray-500">HRV</span>
                          </div>
                          <span className="text-sm font-semibold text-gray-900">{s.rmssd} <span className="text-xs text-gray-500">ms</span></span>
                        </div>
