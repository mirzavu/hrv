...
async function setup() {
    try {
        ...
        // 1. Find existing database or create new one
        let database;
        
        // First, check if database already exists
        console.log(`🔍 Checking for existing database '${DATABASE_NAME}'...`);
        ...
        if (database) {
            ...
            try {
                ...
                console.log(`✅ Database '${DATABASE_NAME}' created successfully (ID: ${database.$id})`);
            } catch (e) {
        ...
        // 2. Find existing collection or create new one
        let collection;
        ...
        if (collection) {
            ...
            try {
                ...
                console.log(`✅ Collection '${COLLECTION_NAME}' created successfully (ID: ${collection.$id})`);
            } catch (e) {
        ...
        // 2.5. Find existing users collection or create new one
        let usersCollection;
        
        console.log(`🔍 Checking for existing collection '${USERS_COLLECTION_NAME}'...`);
        const usersColList = await databases.listCollections(DATABASE_ID);
        usersCollection = usersColList.collections.find(col => col.name === USERS_COLLECTION_NAME);
        
        if (usersCollection) {
            console.log(`✅ Found existing collection '${USERS_COLLECTION_NAME}' (ID: ${usersCollection.$id})`);
        } else {
            console.log(`📦 Creating new collection '${USERS_COLLECTION_NAME}'...`);
            try {
                ...
                console.log(`✅ Users collection '${USERS_COLLECTION_NAME}' created successfully (ID: ${usersCollection.$id})`);
            } catch (e) {
                throw new Error(`Failed to create users collection: ${e.message}`);
            }
        }
